...
Dibangun 2026-08-28, dan dikembangkan serta dipakai sehari-hari di Raspberry Pi 5.
...
**Setup** (opsional, lihat bagian voice note di README): `scripts/setup-whisper.sh` build `whisper.cpp` dari source dan download model ke `bin/whisper-cli` + `data/whisper/ggml-base.bin`. `newTranscriber()` di `cmd/bridge/main.go` cek keberadaan kedua file itu saat startup — kalau tidak ada, transkripsi nonaktif otomatis tanpa bikin bridge gagal start (fitur opsional, bukan hard dependency).
